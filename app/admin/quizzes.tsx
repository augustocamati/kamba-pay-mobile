...
export default function AdminQuizzes() {
  ...
  // Queries
  const { data: quizzesData, isLoading, isError, error } = useQuery({
    ...
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      {isError ? (
        <View style={styles.loading}>
          ...
          <Text style={[styles.loadingText, { color: '#EF4444' }]}>Erro ao carregar quizzes</Text>
          <Text style={{ color: S.sub, marginTop: 8 }}>{ (error as any)?.response?.data?.mensagem || (error as any)?.message }</Text>
          <TouchableOpacity style={[styles.addBtn, { marginTop: 20 }]} onPress={() => queryClient.invalidateQueries({ queryKey: ['admin', 'quizzes'] })}>
            <Text style={styles.addBtnText}>Tentar Novamente</Text>
          </TouchableOpacity>
        </View>
      ) : isLoading ? (
        <View style={styles.loading}>
