...
function NovaTarefaModal({ visible, onClose, onCriar, form, setForm }: NovaTarefaModalProps) {
  return (
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.summaryRow}>
          ...
          <View style={[styles.summaryCard, { flex: 1 }]}>
            ...
            <Text style={styles.summaryCardValue}>{tarefasConcluidas}</Text>
        ...
        {/* Criar Nova Tarefa */}
        <LinearGradient colors={['#3b82f6', '#7c3aed']} style={[styles.actionCard, { marginBottom: 16 }]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
          <View style={{ flex: 1 }}>
        ...
        {/* Criar Nova Campanha */}
        <LinearGradient colors={['#ec4899', '#dc2626']} style={[styles.actionCard, { marginBottom: 16 }]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
          <View style={{ flex: 1 }}>
            <Text style={styles.actionCardTitle}>Criar Nova Campanha</Text>
            <Text style={styles.actionCardSubtitle}>Adicione campanhas de solidariedade para {crianca.nome} ajudar</Text>
            <TouchableOpacity
              style={styles.actionBtn}
            ...
            >
              ...
              <Text style={[styles.actionBtnText, { color: '#be185d' }]}>Criar Campanha</Text>
            </TouchableOpacity>
          </View>
        ...
        <View style={[styles.card, { marginBottom: 16 }]}>
          <Text style={[styles.cardTitle, { marginBottom: 14 }]}>Progresso das Missões</Text>
          {missoes.filter((m) => m.ativa).map((missao) => {
            const progresso = Math.min(
              Math.round((missao.progresso_atual / missao.objetivo_valor) * 100),
              100
            );
            return (
              <View key={missao.id} style={styles.missaoItem}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                  <Text style={{ fontSize: 24 }}>{missao.icone}</Text>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.tarefaTitle}>{missao.titulo}</Text>
                    <Text style={styles.tarefaSubtitle}>{missao.tipo}</Text>
                  </View>
                  <Text style={{ color: '#fff', fontWeight: '700', fontSize: 14 }}>{progresso}%</Text>
                </View>
